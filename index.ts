(function App() {
   String.prototype.padStart = function (
      ...
      if (currStr.length >= targetLength) return this;
      ...
      for (let i = 0; i < nPads; i++) resultStr += padString;
   ...
   // Controller
   const generateWords = function (numberOfWords: number): string[] {
      if (!Number.isInteger(numberOfWords) || numberOfWords <= 0) return;
   ...
   const controlTyping = function (content: string, charPress: string) {
      ...
      if (charPress === ' ') {
         ...
         if (currWordPosition === words.length) controlGetNewWords();
   ...
   // View
   //// Typing Content View
   const renderWords = function (words: string[]) {
      ...
      const html = words
         .map(
            (word, idx) =>
               `<span class="word" data-position="${idx}">${word}</span>`
         )
         .join('');
   ...
   const updateTypingContent = function (transformY: number) {
      const parent = document.querySelector(
         '.typing-content__all'
      ) as HTMLElement;
   ...
   const updateWord = function (className: string, remove: boolean = false) {
      const words = document.querySelectorAll('.word');
      const word = words[currWordPosition] as HTMLElement;
      ...
      if (remove) {
         ...
         if (className === 'word--active') {
            word.dispatchEvent(
               new CustomEvent('wordactive', { bubbles: true })
            );
         }
      }
   };

   const addHandlerLineBreak = function (handler: () => void) {
      const parent = document.querySelector(
         '.typing-content__all'
      ) as HTMLElement;

      parent.addEventListener('wordactive', function (e) {
         const target = (<HTMLElement>e.target).closest('.word') as HTMLElement;

         const currPos = +target.dataset.position;
         if (currPos === 0) return;
         const prevWord = document.querySelectorAll('.word')[
            currPos - 1
         ] as HTMLElement;

         if (prevWord.offsetTop !== target.offsetTop) handler();
      });
   };

   const getLineHeightTypingContent = function (): number {
      const parent = document.querySelector('.word') as HTMLElement;
   ...
   const renderScore = function () {
      const parent = document.querySelector('.score__point') as HTMLElement;
      parent.textContent = `${score}`;
   };

   //// Input Form
   const addHanlerFormKeyDown = function (
      handler: (content: string, charPress: string) => void
   ) {
      const parent = document.querySelector(
         '#typing-field'
      ) as HTMLInputElement;
      ...
      parent.addEventListener('keyup', function (e) {
         if (e.key === 'Backspace') handler(this.value, e.key);
   ...
   const addHandlerFormSubmit = function (handler: () => void) {
      const parent = document.querySelector('.typing-form') as HTMLFormElement;
   ...
   //// Time
   const updateTime = function (minutes: number, seconds: number) {
      const parent = document.querySelector('.timer') as HTMLElement;
